...
count = 0
df = pd.read_csv('AllWeatherDataAgain.csv')
...
for i in range(len(df)):
	...
	params = {
		"latitude": df.Latitude[i], #gets long, lat, and date from dataframe
		"longitude": df.Longitude[i],
		"start_date": df['Event Date'][i],
		"end_date": df['Event Date'][i],
	...
	if(df.Latitude[i]>=-90 and df.Latitude[i]<=90):
...
# #hourly_dataframe.to_excel('hourly_dataframe.xlsx')